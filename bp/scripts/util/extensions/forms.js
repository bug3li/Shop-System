...
import { CHEST_UI_SIZES, custom_content, custom_content_keys, inventory_enabled, number_of_custom_items, } from "./constants.js";
...
class ChestFormData {
    ...
    button(slot, itemName, itemDesc, texture, stackSize = 1, durability = 0, enchanted = false) {
        const targetTexture = custom_content_keys.has(texture)
            ...
            : texture;
        const ID = typeIdToDataId.get(targetTexture ?? "") ??
    ...
    show(player) {
        ...
        for (let i = 0; i < container.size; i++) {
            ...
            const durDamage = durability
                ? Math.round(((durability.maxDurability - durability.damage) /
